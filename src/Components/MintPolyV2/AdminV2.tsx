...
const AdminPage = () => {
  ...
  const [claimsData, setClaimsData] = useState<{ address: string; claimCount: number }[]>([]);
    ...
    if (walletProvider) {
      const fetchData = async () => {
        ...
        try {
          ...
          const claimsData = addresses.map((address: string, index: number) => ({
            address,
    ...
    <Box p={4}>
      ...
      <Box mb={4}>
        ...
        <Table variant="simple" size="sm">
          ...
          <Tbody>
            {claimsData.map(({ address, claimCount }) => (
              <Tr key={address}>
