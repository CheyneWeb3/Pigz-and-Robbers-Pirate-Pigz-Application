...
contract NFTRegistration {
    ...
    // New mapping to track the number of claims per user
    ...
    function claimReward() public {
        ...
        // Ensure the user can only claim for a valid set once
        require(userClaimCounts[msg.sender] < claimCount, "All available claims have been claimed");

        uint256 unclaimedCount = claimCount - userClaimCounts[msg.sender];
        uint256 rewardAmount = unclaimedCount * REWARD_PER_CLAIM;

        // Update the claim count for the user
        userClaimCounts[msg.sender] += unclaimedCount;
