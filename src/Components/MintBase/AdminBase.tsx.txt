...
const AdminPage = () => {
    ...
    if (walletProvider) {
      const fetchData = async () => {
        ...
        try {
          ...
          const calculatedTotalClaimValue = totalClaimCount * 0.003334; // 20 ETH (Base) per valid claim
  ...
  return (
    <Box>
      ...
      <Text fontSize="2xl" mb={4}>Base Pigz Admin Panel</Text>

      <Box mb={4}>
        <Text fontSize="lg">Contract Balance: {contractBalance} ETH (Base)</Text>
      </Box>
      <Box mb={4}>
        <Text fontSize="lg">Total Claim Value Payable: {totalClaimValue} ETH (Base)</Text>
      </Box>
